...
1. Decide what you want to call your command. This will be yourCommandName.

2. Run `/mycmd check yourCommandName` to see if someone has already made a command with yourCommandName. It should tell you "No command found with this ID." 
...
5. Now we can start adding the commands that will run when you run `/yourCommandName`. We will use yourFirstCommand to symbolize the entire command that you would normally type into the chat. To add a command, run `/mycmd-edit yourCommandName runcmd add yourFirstCommand`

6. Repeat step 5 until all your commands are added (e.g. yourFirstCommand yourSecondCommand etc...
